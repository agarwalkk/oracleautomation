 ...
 *
 * <p>The router is the one place that crosses layer boundaries: extraction
...
final class CommandRouter {
    ...
    static String dispatch(AgentCommand cmd) throws Exception {
        switch (cmd.getCommand()) {
            case "health":     return Results.health();

            case "scan":       return DomJson.scanResult(DomScanner.scan(false), "scan");
            case "raw":        return DomJson.scanResult(DomScanner.scan(true), "raw");
            case "layout":     return TextLayout.write(DomScanner.scan(false));
            case "tables":     return DomJson.tables(DomScanner.detectTables());

            case "focus":        return ActionExecutor.executeFocus(cmd);
            case "click":        return ActionExecutor.executeClick(cmd);
            case "doubleclick":  return ActionExecutor.executeDoubleClick(cmd);
            case "settext":      return ActionExecutor.executeSetText(cmd);
            case "clear":        return ActionExecutor.executeClear(cmd);
            case "selectoption": return ActionExecutor.executeSelectOption(cmd);
            case "setcheck":     return ActionExecutor.executeSetCheck(cmd);
            case "expandtree":   return ActionExecutor.executeExpandTree(cmd);
            case "collapsetree": return ActionExecutor.executeExpandTree(cmd);
