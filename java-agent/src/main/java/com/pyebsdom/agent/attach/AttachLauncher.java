package com.pyebsdom.agent.attach;
 ...
 *
 * <p>Attaches to a running JVM process, loads {@code ebs-dom-agent.jar} into
 * it with the given argument string, then immediately detaches.
 *
 * <p>{@link VirtualMachine#loadAgent(String, String)} is synchronous: it does
 ...
 *        com.pyebsdom.agent.AttachLauncher \
 ...
 * <h3>Exit codes</h3>
 * <ul>
 *   <li>{@code 0} — agent loaded and executed (result written to {@code out} file).</li>
...
public final class AttachLauncher {
    ...
    private AttachLauncher() { /* main-class entry point only */ }
    ...
    private static void printPreLoadDiagnostics(Class<?> vmClass, Object vm, String agentJar) {
        ...
        if (jarFile.isFile()) {
            ...
            printClassMajor(agentJar, "com.pyebsdom.agent.AgentMain");
            printClassMajor(agentJar, "com.pyebsdom.agent.execute.ActionExecutor");
            printClassMajor(agentJar, "com.pyebsdom.agent.attach.AttachLauncher");
        }
    }
    ...
    private static void printUsage() {
        ...
        System.err.println("       com.pyebsdom.agent.AttachLauncher \\");
