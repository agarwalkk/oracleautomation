...
/**
 * The manifest entry point for dynamic attach (Premain-Class / Agent-Class).
 *
 * <p>This class is intentionally <b>self-contained</b>: it depends on no other
 * agent class (not even the JSON helpers), so Java 8 can always load it even if
 * the full agent has a verification or linkage problem. It then reflects into
 * {@link AgentMain#runCommand} to do the real work. The small private
 * {@code quote()} below is a deliberate duplicate of the JSON escaper for the
 * same reason — a linkage failure must still be able to write an error file.
 */
public final class AgentBootstrap {
    ...
    private static void run(String agentArgs, Instrumentation inst) {
        ...
        try {
            Class<?> agentClass = Class.forName("com.pyebsdom.agent.AgentMain");
