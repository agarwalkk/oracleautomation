...
public final class DomScanner {
    ...
    /**
     * Runs the {@link TableDetector} on the EDT and returns the detected
     * tables. Serialisation is the responsibility of {@code render.DomJson}
     * so that {@code extract} never depends on {@code render}.
     */
    public static List<TableModel> detectTables() throws Exception {
    ...
    private static DomNode buildNode(Component comp,
        ...
        try {
            Map<String, String> refMap = ComponentReader.extract(comp);
            ...
            List<String> options = ComponentReader.extractOptions(comp, 100);
            if (!options.isEmpty()) {
            ...
            if ("ListView".equals(node.simpleClassName)) {
                try {
                    treeRows = ComponentReader.extractListViewRows(comp, 256);
            ...
            if (treeRows == null || treeRows.isEmpty()) {
                try {
                    treeRows = ComponentReader.extractTreeRows(comp, 256);
            ...
            List<String> tabTitles = ComponentReader.extractTabTitles(comp, 32);
            if (!tabTitles.isEmpty()) {
                ...
                String selectedTabTitle = ComponentReader.extractSelectedTabTitle(comp, tabTitles);
                ...
                String tabStates = ComponentReader.extractTabStates(comp, tabTitles.size());
                if (tabStates != null) {
        ...
        if ("Menu".equals(node.semanticType)) {
            try {
                List<String> menuItems = ComponentReader.extractMenuItems(comp, 64);
            ...
            try {
                List<String> accItems = ComponentReader.extractAccessibleMenuItems(comp, 64);
                if (!accItems.isEmpty()) {
    ...
    public static final class ScanResult {
        ...
        // Serialisation lives in render.DomJson.scanResult(result, command)
        // so that the extract layer stays free of any JSON concerns.
    }
}
