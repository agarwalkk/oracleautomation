...
import com.pyebsdom.agent.extract.Identity;
import javax.swing.*;
import java.awt.*;
...
public final class ComponentResolver {
    ...
    // ── Live identity reconstruction ──────────────────────────────────────
    // Delegated to extract.Identity — the single implementation shared with
    // scan-time identity. These thin wrappers keep the strategy call sites
    // below readable; there is no second copy of the logic any more.

    private static String liveSemanticId(Component c, List<Component> all) {
        return Identity.liveSemanticId(c, all);
    }

    private static String liveScopeId(Component c, List<Component> all) {
        return Identity.liveScopeId(c, all);
    }

    private static String canonicalLabel(Component c) {
        return Identity.canonicalLabel(c);
    }
    ...
    public static String componentJson(Component comp) {
        ...
        sb.append('{');
        sb.append("\"className\":").append(Json.quoted(comp.getClass().getName())).append(',');
        sb.append("\"simpleName\":").append(Json.quoted(comp.getClass().getSimpleName())).append(',');
        sb.append("\"name\":").append(Json.quoted(comp.getName())).append(',');
        sb.append("\"accessibleName\":").append(Json.quoted(accessibleName(comp))).append(',');
        sb.append("\"text\":").append(Json.quoted(componentText(comp))).append(',');
    ...
    private static String accessibleName(Component comp) {
        return Identity.accessibleName(comp);
    }
