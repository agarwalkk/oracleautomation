...
public final class DomScanner {
    ...
    private static DomNode buildNode(Component comp,
        ...
        // Diagnostic: dump class hierarchy, methods, and fields for Tree nodes
        if ("Tree".equals(node.semanticType)) {
            try {
                StringBuilder diag = new StringBuilder();
                Class<?> cls = comp.getClass();
                // Class hierarchy
                diag.append("HIERARCHY:");
                Class<?> c = cls;
                while (c != null) {
                    diag.append(" ").append(c.getName());
                    c = c.getSuperclass();
                }
                // All declared methods across hierarchy (non-Object)
                diag.append(" | METHODS:");
                c = cls;
                java.util.Set<String> seen = new java.util.LinkedHashSet<>();
                while (c != null && !c.getName().equals("java.lang.Object")) {
                    for (java.lang.reflect.Method dm : c.getDeclaredMethods()) {
                        String sig = dm.getName() + "(" + dm.getParameterCount() + ")";
                        seen.add(sig);
                    }
                    c = c.getSuperclass();
                }
                for (String s : seen) diag.append(" ").append(s);
                // All declared fields across hierarchy (non-Object)
                diag.append(" | FIELDS:");
                c = cls;
                while (c != null && !c.getName().equals("java.lang.Object")) {
                    for (java.lang.reflect.Field df : c.getDeclaredFields()) {
                        diag.append(" ").append(c.getSimpleName()).append(".").append(df.getName());
                    }
                    c = c.getSuperclass();
                }
                node.attributes.put("_treeDiag", diag.toString());
            } catch (Throwable diagErr) {
                node.attributes.put("_treeDiagError", diagErr.getClass().getName() + ": " + diagErr.getMessage());
            }
        }
